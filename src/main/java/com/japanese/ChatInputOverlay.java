...
class ChatInputOverlay extends Overlay //remove abstract when actually making overlays with this
{
    ...
    public Dimension render(Graphics2D graphics) {
        String msg = plugin.romToJap.chatMsg;
        ...
        panelComponent.setPreferredSize(new Dimension(Math.min(14*(msg.length() + 1)+8*5,inputWidth),0));
            ...
            for (int i = 0;i < newMsgs.length; i++) {
                if (i == newMsgs.length - 1) {
                    panelComponent.getChildren().add(LineComponent.builder()
                            .left(newMsgs[i])
                            .right("(" + Integer.toString(msg.length()) + "\n/80)")
                            .build());
                } else {
                    panelComponent.getChildren().add(LineComponent.builder()
                            .left(newMsgs[i])
